...
export async function GET() {
  try {
    const r = await db().query("SELECT enabled, facilities_enabled, doctors_enabled, concurrency, tasks_per_tick, facilities_tasks_per_tick, doctors_tasks_per_tick, heartbeat_at, jina_keys FROM settings WHERE id=1");
    const row = r.rows[0] ?? {};
...
// PATCH /api/settings { enabled?, facilities_enabled?, doctors_enabled?, concurrency 1-10?, tasks_per_tick 1-50?, facilities_tasks_per_tick 1-50?, doctors_tasks_per_tick 1-50?, jina_keys? (multiline, one per line, replaces all) }
